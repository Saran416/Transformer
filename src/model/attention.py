...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, input_dim, num_heads, mask=False):
        ...
        self.head_dim = input_dim // num_heads  # d
        self.mask = mask
    ...
    def forward(self, X):  # X: (N, S, D), mask: (N, 1, S, S) or (N, S, S)
        N, S, D = X.shape
        H = self.num_heads
        d = self.head_dim

        Q = self.W_q(X).view(N, S, H, d).transpose(1, 2) # (N, H, S, d)
        K = self.W_k(X).view(N, S, H, d).transpose(1, 2) # (N, H, S, d)
        V = self.W_v(X).view(N, S, H, d).transpose(1, 2) # (N, H, S, d)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, X, context):  # X: (N, T, D), context: (N, S, D)
        N, T, _ = X.shape
        S = context.size(1)

        Q = self.W_q(X).view(N, T, self.num_heads, self.head_dim).transpose(1, 2)       # (N, H, T, d)
        K = self.W_k(context).view(N, S, self.num_heads, self.head_dim).transpose(1, 2) # (N, H, S, d)
        V = self.W_v(context).view(N, S, self.num_heads, self.head_dim).transpose(1, 2) # (N, H, S, d)

        scores = torch.matmul(Q, K.transpose(-2, -1)) / self.scale  # (N, H, T, S)
        weights = F.softmax(scores, dim=-1)
        out = torch.matmul(weights, V)  # (N, H, T, d)

        out = out.transpose(1, 2).contiguous().view(N, T, self.model_dim)  # (N, T, D)
        return self.out_proj(out)
